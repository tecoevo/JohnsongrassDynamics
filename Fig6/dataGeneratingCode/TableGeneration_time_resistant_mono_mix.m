...
% Generates the average time till a resistant mutant, a mutant fully 
% resistant against one of the herbicides, a double mutant and a fully 
% resistant double mutant establishes in a Johnsongrass population treated 
% with ACCaseinhibitor or a mixture of ACCase- and ALS-inhibitor with same 
% efficacy and the proportion of simulated populations in whish 
% these mutants establish.
...
% column number) of herbicide cycling: 
time_double_resistance = zeros(2, 2);
% 2 x 10 array with average time till double resistance, i.e. survival of
% first plant fully resistant against both herbicides, with (row 2)  
% and without tillage (row 1) for different cycle length (equaling 
% column number) of herbicide cycling: 
time_full_double_resistance = zeros(2, 2);
...
for til = 1:2
    ...
    herb1 = ones(1, n_years);

    % Loop over mono treatment and mixture
    for l = 1:2
        %   1 x n_years vektor of herbicide application. Each entry 
        %   corresponds to one season and is a logical value stating whether 
        %   ALS-inhibitor is applied
        herb2 = (l==1) * ones(1, n_years);
        ...
        for i = 1:n_rep
            ...
            % one of the herbicides survives:
            ...
            % against one of the herbicides survives:
            ...
            % against both herbicides survives:
...
T.Tillage = reshape(repmat([0;1], 1, 2), [], 1);
T.CycleLength = reshape(repmat([0, 30], 2, 1), [], 1);
...
writetable(T, 'Table_Stochastic_Resistance_Mono_Mix');