...
% Generates the average time till the fisrst mutant, a single resistant 
% mutant, a mutant fully resistant against one of the herbicides, a double 
% mutant and a fully resistant double mutant establishes in a Johnsongrass
% population treated with ACCase- and ALS-inhibitor with same efficacy 
% cycled depending on the cycle length and the proportion of simulated 
% populations in whish these mutants establish.
...
% first plant fully resistant against at least one of the herbicides, with 
% (row 2) and without tillage (row 1) for different cycle length (equaling 
% column number) of herbicide cycling: 
time_escape = zeros(2, 10);
% 2 x 10 array with average time till resistance, i.e. survival of
% first plant resistant against at least one of the herbicides, with   
% (row 2) and without tillage (row 1) for different cycle length (equaling 
% column number) of herbicide cycling: 
time_resistance = zeros(2, 10);
% 2 x 10 array with average time till single resistance, i.e. survival of
% first plant resistant against one of the herbicides, with   
% (row 2) and without tillage (row 1) for different cycle length (equaling 
% column number) of herbicide cycling: 
time_single_resistance = zeros(2, 10);
% 2 x 10 array with average time till double resistance, i.e. survival of
% first plant resistant against both herbicides, with (row 2)  
% and without tillage (row 1) for different cycle length (equaling 
% column number) of herbicide cycling: 
time_double_resistance = zeros(2, 10);
% 2 x 10 array with average time till double resistance, i.e. survival of
% first plant fully resistant against both herbicides, with (row 2)  
% and without tillage (row 1) for different cycle length (equaling 
% column number) of herbicide cycling: 
time_full_double_resistance = zeros(2, 10);

% 2 x 10 array with number of escapes from control, with (row 2) and 
% without tillage (row 1) for different cycle length (equaling 
% column number) of herbicide cycling: 
n_escape = zeros(2, 10);
% 2 x 10 array with number of populations that evolve resistance, with
% (row 2) and without tillage (row 1) for different cycle length (equaling 
% column number) of herbicide cycling: 
n_resistance = zeros(2, 10);
% 2 x 10 array with number of populations that evolve single resistance, 
% with (row 2) and without tillage (row 1) for different cycle length 
% (equaling  column number) of herbicide cycling: 
n_single_resistance = zeros(2, 10);
% 2 x 10 array with number of populations that evolve double resistance, 
% with (row 2) and without tillage (row 1) for different cycle length 
% (equaling  column number) of herbicide cycling:  
n_double_resistance = zeros(2, 10);
% 2 x 10 array with number of populations that evolve full double 
% resistance, with (row 2) and without tillage (row 1) for different 
% cycle length (equaling  column number) of herbicide cycling: 
n_full_double_resistance = zeros(2, 10);

% 2 x 10 array with final density of populations that escaped from control
% with (row 2) and without tillage (row 1) for different cycle length 
% (equaling column number) of herbicide cycling: 
final_density = zeros(2, 10);
...
for til = 1:2
    ...
    % Loop over different cycling lentgh
    for l = 1:10
        ...
        for i = 1:n_rep
            ...
            SB(:, 1) = multinomrand(dens_seeds * A, ...
            ...
            R(:, 1) = multinomrand(dens_rhizomes * A, ...
                      repmat([1-RR1-RW1; RW1; RR1], 3, 1) .* ...
                      repelem([1-RR2-RW2; RW2; RR2], 3, 1));
            ...
            % 1 x 9 logical vector stating if a  plant of the corresponding 
            % genotype survived till reproduction in any season, 
            % 1 x 9 vector giving the first season or 1 otherwise:
            [genotypes, season] = max(logical(P'),[],1);
            % 1 x 9 Vector of season in which the first plant of the
            % corresponding genotype survived till reproduction, where NaN 
            % means no plant of that genotype established:
            season = genotypes .* season;
            season(~genotypes) = NaN;

            % Time till first plant resistant against herbicides survives:
            time_resistance(til, l) = time_resistance(til, l) + max(min(season(2:end)), 0);
            % Number of replicates with a plant resistant against 
            % the herbicides surviving:
            n_resistance(til, l) = n_resistance(til, l) + any(genotypes(2:end));

            % Time till first plant resistant against one of the
            % herbicides survives:
            time_single_resistance(til, l) = time_single_resistance(til, l) + max(min(season([2,3,4,7])), 0);
            % Number of replicates with a plant resistant against 
            % one of the herbicides surviving:
            n_single_resistance(til, l) = n_single_resistance(til, l) + any(genotypes([2,3,4,7]));

            % Time till first plant fully resistant against at least one 
            % of the herbicides survives:
            time_escape(til, l) = time_escape(til, l) + max(min(season([3,6,7,8,9])), 0);
            % Number of replicates with a plant fully resistant 
            % against at least one of the herbicides surviving:
            n_escape(til, l) = n_escape(til, l) + any(genotypes([3,6,7,8,9]));

            % Time till first plant resistant against both
            % herbicides survives:
            time_double_resistance(til, l) = time_double_resistance(til, l) + max(min(season([5,6,8,9])), 0);
            % Number of replicates with a plant resistant 
            % against both herbicides surviving:
            n_double_resistance(til, l) = n_double_resistance(til, l) + any(genotypes([5,6,8,9]));

            % Time till first plant fully resistant against
            % both herbicides survives:
            time_full_double_resistance(til, l) = time_full_double_resistance(til, l) + season(9);
            % Number of replicates with a plant fully resistant 
            % against both herbicides surviving:
            n_full_double_resistance(til, l) = n_full_double_resistance(til, l) + genotypes(9);

            % Final density of populations that escaped from control:
            final_density(til, l) = final_density(til, l) + sum(P(:,end)) * any(genotypes([3,6,7,8,9]));
        end
    end
end
...
T.AverageResistanceYear = reshape(time_resistance(:, :), [], 1) ./ reshape(n_resistance(:, :), [], 1);
T.FractionResistance = reshape(n_resistance(:, :), [], 1) / n_rep;
T.AverageEscapeYear = reshape(time_escape(:, :), [], 1) ./ reshape(n_escape(:, :), [], 1);
T.FractionEscapes = reshape(n_escape(:, :), [], 1) / n_rep;
T.AverageSingleResistanceYear = reshape(time_single_resistance(:, :), [], 1) ./ reshape(n_single_resistance(:, :), [], 1);
T.FractionSingleResistance = reshape(n_single_resistance(:, :), [], 1) / n_rep;
T.AverageDoubleResistanceYear = reshape(time_double_resistance(:, :), [], 1) ./ reshape(n_double_resistance(:, :), [], 1);
T.FractionDoubleResistance = reshape(n_double_resistance(:, :), [], 1) / n_rep;
T.AverageFullDoubleResistanceYear = reshape(time_full_double_resistance(:, :), [], 1) ./ reshape(n_full_double_resistance(:, :), [], 1);
T.FractionFullDoubleResistance = reshape(n_full_double_resistance(:, :), [], 1) / n_rep;
T.FinalDensityEscapes = reshape(final_density(:, :), [], 1) ./ reshape(n_escape(:, :), [], 1) / A;
% write table to text file 
writetable(T, 'Table_Stochastic_Resistance_CycleLength2');